...
public abstract class AbstractUploadController {
    ...
    protected MultipartFile getMultipart(HttpServletRequest request) {
        ...
        // 优先获取名为 file 的部件。视频上传时会附带名为 image 的截图部件，且截图部件位于视频部件之前，不能误取
        MultipartFile file = fileMap.get("file");
        if (file != null) {
            return file;
        }
        return fileMap.entrySet().stream()
                .filter(entry -> !"image".equals(entry.getKey()))
                .map(Map.Entry::getValue)
                .findFirst()
                .orElseThrow(() -> new LogicException("Upload file not found"));
    }
    ...
    protected interface ExtraHandle {
        ...
        void handle(File tempFile, String baseName, String extension, String pathname, FileHandler fileHandler,
                    Site site, Long userId, Map<String, Object> result) throws IOException;
    }
