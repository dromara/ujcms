...
import com.ujcms.cms.core.support.ApplicationContextHolder;
...
import com.ujcms.common.web.HtmlParserUtils;
import com.ujcms.common.web.PageUrlResolver;
...
public class Channel extends GeneratedChannel implements PageUrlResolver, Anchor, TreeEntity {
    ...
    public String getDynamicUrl(int page) {
        ...
        String url = UrlBuilder.of(getSite().getDynamicUrl()).appendPath(prefix).appendPath(getAlias()).toString();
        if (page <= 1) {
            return url;
        }
        return url + "/" + page;
    ...
    private Optional<Channel> getFirstChannel(Long channelId) {
        return Optional.ofNullable(ApplicationContextHolder.getApplicationContext())
    ...
    public String getDynamicFullUrl(int page) {
        ...
        if (getType() == TYPE_LINK_CHILD) {
            return getFirstChannel(getId()).map(channel -> channel.getDynamicFullUrl(page)).orElse("");
        }
        String prefix = Optional.ofNullable(getSite().getConfig().getChannelUrl()).orElse(UrlConstants.CHANNEL);
        String url = UrlBuilder.of(getSite().getDynamicFullUrl()).appendPath(prefix).appendPath(getAlias()).toString();
        if (page <= 1) {
            return url;
        }
        return url + "/" + page;
    ...
    public String getStaticFullUrl(int page) {
        String linkUrl = getLinkUrl();
        if (getType() == TYPE_LINK && StringUtils.isNotBlank(linkUrl)) {
            return getSite().assembleLinkUrl(linkUrl);
        }
        if (getType() == TYPE_LINK_CHILD) {
            return getFirstChannel(getId()).map(channel -> channel.getStaticFullUrl(page)).orElse("");
        }
        if (page > getSite().getHtml().getListPages()) {
            return getDynamicFullUrl(page);
        }
        return Contexts.isMobile()
                ? getSite().getMobileStaticFullUrl(getChannelUrl(page))
                : getSite().getNormalStaticFullUrl(getChannelUrl(page));
    }
    ...
    // region Associations
    /**
     * 是否有子栏目。只有在查询栏目列表时有效；查询栏目对象时无效，请使用 `channel.children?size > 0` 判断
     */
    @Nullable
