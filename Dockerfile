...
WORKDIR /builder
ARG JAR_FILE=ujcms-starter/target/*.jar
COPY ${JAR_FILE} ujcms.jar
RUN java -Djarmode=tools -jar ujcms.jar extract --layers --destination extracted
COPY ujcms-starter/src/main/resources/application-docker.yaml ./extracted/application/config/application.yaml
...
COPY ujcms-starter/src/main/resources/application-docker.yaml ./config/application.yaml
# 将初始文件拷贝至 /usr/src/ujcms，再由初始化脚本复制到 /ujcms/static，避免文件内容被挂载目录覆盖
COPY ujcms-starter/src/main/webapp/ /usr/src/ujcms/
