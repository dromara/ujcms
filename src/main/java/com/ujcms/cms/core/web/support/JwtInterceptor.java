...
public class JwtInterceptor implements HandlerInterceptor {
    ...
    public JwtInterceptor(JwtProperties properties, UserService userService, Algorithm algorithm) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (accessToken != null && accessToken.startsWith(properties.getHeaderSchema())) {
            ...
            try {
                // 使用SM4解密
                accessToken = Secures.sm4Decrypt(accessToken, properties.getTokenSecret());
                ...
                // Base64 解码失败
                logger.debug("Base64 decode failed: " + accessToken, e);
            } catch (BadPaddingException | IllegalBlockSizeException e) {
                // SM4 解密失败
                logger.debug("SM4 Decrypt failed: " + accessToken, e);
            }
        }
        return true;
    }
